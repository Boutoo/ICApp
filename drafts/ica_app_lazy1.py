...
class MyApp(QWidget):
    ...
    def go_to_page(self):
        ...
        if 0 <= page_number < len(self.data):
    ...
    def switch_page_and_plot(self, i):
        ...
        if self.plots[i-1] is None:
            self.plots[i-1] = self.create_ica_plot(i-1)

            # Hide loading
            loading_label = self.loading_labels[i-1]
            loading_label.hide()

            # Adding widget
            self.stacked_widget.widget(i).layout().addWidget(self.plots[i-1])
